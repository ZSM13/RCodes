...
PATIENTS2<-reshape::rename(PATIENTS2, c(Subscriber.ID="SUBSCRIBER_ID"))

# Identifies the columns for the file to be exported
PATIENTS2<-PATIENTS2[,c("SUBSCRIBER_ID", "AdmitDate")]

#Export csv file#
write.csv(PATIENTS2, (file=paste ("DailyMPT-Insurance-", format(Sys.Date(), "-%Y-%m-%d"), ".csv", sep="")), row.names=FALSE)
